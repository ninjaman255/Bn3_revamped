...
Net:on("player_connect", function(event)
    player_maps[event.player_id] =
    'default'                                --TODO this does not work correctly if the player is transfered elsewhere while joining
    ...
    for i, area_id in ipairs(areas) do
        ...
        for j, object_id in ipairs(objects) do
            ...
            if address and port then
                map_info.r[address .. ":" .. port] = { data = custom_p["Data"], incoming_data = custom_p
                ["Incoming Data"] }
            end
