...
local admin_nickname_list = { "D3str0y3d" }
...
local function debug_gift_admin_key(player_id)
  local player_name = Net.get_player_name(player_id)
  if (player_name == admin_nickname_list[0]) then
    Net.give_player_item(player_id, 0)
    print(player_name == admin_nickname_list[0])
  end
end
...
function handle_player_connect(player_id)
  ...
  -- Debugging/Temp gift me admin on boards.
  debug_gift_admin_key(player_id)
